...
export function createBetterPatchTool(ctx: OpenClawPluginToolContext, resolveSandbox?: SandboxResolver): AnyAgentTool | null {
  ...
  return {
    name: "better_patch",
    label: "Better Patch",
    description: "Apply a patch to files. Update contents must be valid UTF-8. Deletion accepts binary files and empty directories; missing paths succeed, and non-empty directories are not removed. Use *** Begin Patch and *** End Patch, with *** Add File: path (lines prefixed +), *** Delete File: path, or *** Update File: path. Updates accept optional *** Move to: path, @@ or @@ context anchors, and lines prefixed space (context), - (remove), + (add). Insertion-only chunks with @@ context insert immediately after the matched anchor; without a textual anchor they append at the file ending. *** End of File anchors a chunk at EOF without overlapping earlier chunks. Moves to the same resolved path are updates. Operations run in written order; preflight validates against preceding simulated changes. Unchanged adds/updates and missing deletes skip mutations. Results summarize net A/M/D changes and N unchanged paths. Paths are relative to the agent workspace unless absolute. Matching ignores LF, CR, CRLF, and LFCR terminators and a leading file BOM. The original BOM stays at the file start even when all text is deleted by an update. Context matching tolerates whitespace and common Unicode punctuation while preserving source context text. Anchors and chunk patterns must match exactly once at the first tolerance level with any matches; ambiguous matches reject before edits. Added lines inherit the preceding output line ending (original first line at the start, LF fallback). Other source endings stay unchanged except at EOF: the new last line inherits the original final ending, including no newline, except that explicitly added empty lines are retained and terminated as needed. Send patch text in the input field, not a shell command. Changes are not transactional; I/O failures can leave partial edits.",
    parameters: {
      type: "object",
